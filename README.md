# Islamly - Universal Scholarly Infrastructure

This NextJS prototype is a comprehensive portal for authentic Islamic knowledge, built with a focus on global reach and scholarly accuracy.

## Core Features Implemented

### 1. Universal Knowledge Hub
- **Quran Reader**: High-fidelity Uthmani script with real-time translation support.
- **10 Authentic Qira'at**: Integrated selection for canonical variant readings (Hafs, Warsh, etc.).
- **7709+ Language Support**: Dynamic translation selector pulling from global resource databases.

### 2. Intelligent Scholarly Tools (AI-Powered)
- **Ask AI Assistant**: Genkit-integrated search across a verified knowledge hub of 60 scholarly modules.
- **Passage Simplifier**: AI tool to explain complex classical texts for students of knowledge.

### 3. Practical Living Systems
- **Dynamic Prayer Schedule**: Auto-location (GPS) detection with manual city search fallback.
- **Calculation Methods**: Supports MWL, ISNA, Umm Al-Qura, and other major scholarly standards.
- **Masjid & Halal Locators**: Location-aware discovery tools for local community services.

### 4. Community & Life-Stage Portals
- **The Muslim Household**: Family planning and marriage fiqh resources.
- **Kids Corner**: Interactive learning for children.
- **Youth Hub (Teens)**: Real-talk sessions and community events for teenagers.
- **Reverts Welcome**: Dedicated onboarding and mentor connection for new Muslims.
- **Islamic News**: Curated global Ummah updates and scholarly announcements.

### 5. Advanced Education
- **Fiqh Navigator**: Structured jurisprudence guide across major topics.
- **Scholarly Defense**: Evidence-based refutations of common misconceptions.
- **Daily Adhkar**: Morning and evening remembrance tracker.
- **Islamly Live**: Simulated 24/7 Quran radio and scholarly stream reminders.

### 6. Security & Infrastructure
- **Unified Auth**: Support for Google, Apple, Discord, GitHub, Microsoft, and Email.
- **Auth Guard**: Full-app locking to ensure private, personalized data remains secure.
- **Firestore Profiles**: User-specific preferences (like language) saved across sessions.

## Tech Stack
- **Framework**: Next.js 15 (App Router)
- **UI**: Shadcn/UI, Tailwind CSS, Lucide Icons
- **Backend**: Firebase (Auth, Firestore)
- **AI**: Genkit (Google Gemini)
