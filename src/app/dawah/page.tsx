...
const dawahModules = [
  { id: 1, title: "The Concept of God", cat: "Theology", desc: "Explaining Tawhid to a secular audience." },
  { id: 2, title: "Women in Islam", cat: "Societal", desc: "Refuting misconceptions about status and rights." },
  { id: 3, title: "Purpose of Life", cat: "Philosophical", desc: "Using the Quran to address existential questions." },
  { id: 4, title: "Prophethood Proofs", cat: "Evidence", desc: "Scientific and historical evidence for Muhammad (PBUH)." },
];
...
export default function DawahPage() {
  ...
  const handleComingSoon = () => {
    toast({ title: "Coming Soon", description: "Resource node is being synchronized for download." });
  };
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 pb-20 max-w-2xl mx-auto">
      ...
      <div className="grid gap-4">
        ...
        {dawahModules.map((m) => (
          <Card key={m.id} className="glass-card group hover:border-rose-500/50 transition-all cursor-pointer overflow-hidden" onClick={handleComingSoon}>
      ...
      <section className="grid grid-cols-2 gap-4">
        <Card className="glass-card p-6 text-center border-white/5 space-y-3">
          ...
          <Button variant="outline" size="sm" className="w-full text-[9px] font-black uppercase h-8" onClick={handleComingSoon}>Get PDF</Button>
        </Card>
        <Card className="glass-card p-6 text-center border-white/5 space-y-3">
          ...
          <Button variant="outline" size="sm" className="w-full text-[9px] font-black uppercase h-8" onClick={handleComingSoon}>Share Node</Button>
        </Card>
      </section>
