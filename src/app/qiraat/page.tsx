...
import { QIRAAT_DATA } from '@/lib/qiraat-data';
import { BASE_QURAN, VARIANTS } from '@/lib/quran-content';
import { getRiwayahText, QuranAyah } from '@/lib/quran-data';

type Riwayah = (typeof QIRAAT_DATA)[number]['name'];

const QIRAAT_HIERARCHY = QIRAAT_DATA.reduce((acc, qiraah) => {
  const existing = acc.find((item) => item.imam === qiraah.reciter);
  if (existing) {
    existing.riwayahs.push(qiraah.name);
  } else {
    acc.push({ imam: qiraah.reciter, riwayahs: [qiraah.name] });
  }
  return acc;
}, [] as { imam: string; riwayahs: string[] }[]);
...
export default function QiraatReaderPage() {
  ...
  const renderAyahText = (ayah: QuranAyah) => {
    const variantText = getRiwayahText(
      ayah.text,
      VARIANTS,
      ayah.surah,
      ayah.ayah,
    ...
    const hasDiff =
      selectedRiwayah !== "Hafs 'an 'Asim" && variantText !== hafsText;
